...
public class getUserData extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(first == false){ //최초실행일 경우
            ...
            Boolean[] isClicked = {false, false, false, false, false, false, false, false, false, false, false, false};

            //질병 버튼 클릭 시
            btn1.setOnClickListener(new View.OnClickListener(){
                @Override
                public void onClick(View v) {
                    if(isClicked[0]==false){    //버튼이 선택되지 않았으면
                        //선택됨으로 바꾸고
                        isClicked[0]=true;
                        //버튼 색 변경 후
            ...
            completeBtn.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    //라디오 버튼을 체크 했는지 확인
                    if(radioNo.isChecked()==true || radioYes.isChecked()==true){    //체크 했으면
                        //최초실행 했다고 저장
            ...
            //최초실행이 아니면 뷰를 보여주지 않고 OCRMainActivity 실행
